...
<template>
  <div class="file-manager">
    <div v-for="(folder, index) in folders" :key="folder.id" class="toggle-wrapper">
      ...
      <div class="toggle-content">
        <div class="pt-2 flex flex-col gap-2">
          <div v-for="file in folder.files" class="file" @click="selectFile(folder.id, file.id)">
            <div>
              <svg v-if="file.type === 'md'" width="17" height="16" viewBox="0 0 17 16" fill="none"
            ...
            <span class="title" :class="{'!text-white': file.status === 'open'}">
              {{ file.title }}
            </span>
          </div>
        </div>
